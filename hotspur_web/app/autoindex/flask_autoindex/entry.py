...
class Directory(Entry):
    ...
    def __init__(self, path, rootdir=None, autoindex=None):
        super(Directory, self).__init__(path, rootdir, autoindex)
        self.type='Directory'

    def explore(self, sort_by='name', order=1, show_hidden=False, rule=lambda x: True):
        ...
        for ent in entries:
            if (show_hidden or not ent.hidden) and rule(ent):
                yield ent
